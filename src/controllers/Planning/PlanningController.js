...
class PlanningController {
  deletePlanning(request, response) {
    ...
    database
      .raw("exec stp_cdpd_delete_planejamento ?", [id])
      .then((data) => {
        console.log(data);
        response.json(data);
      })
  ...
  AlterProfile(request, response) {
    const { idPlanning, idProfile } = request.body;

    database
      .raw("exec stp_altera_perfil ?,?", [idPlanning, idProfile])
      .then((data) => {
        console.log(data);
        response.json(data);
      })
  ...
  MacrocicloNew(request, response) {
    ...
    database
      .raw("exec stp_cdpd_planejamento_macrociclo_criacao ?,?,?,?,?", [
        Month1,
        Month2,
        Month3,
        IdPlanningMacrociclo,
  ...
  async get(request, response) {
    ...
    var resultsPromise = planejamento.map(async (obj) => {
      ...
      planejamento_anamnese = await database
        .select(
          "id",
      ...
      planejamento_historico_patologico = await database
        .select(
          "id",
          "id_planejamento as idPlanning",
      ...
      planejamento_historico_patologico = await Promise.all(
        planejamento_historico_patologico,
      );
      ...
      planejamento_avaliacao_funcional = await database
        .select(
          "id",
          "id_planejamento as idPlanning",
      ...
      planejamento_avaliacao_funcional = await Promise.all(
        planejamento_avaliacao_funcional,
      );
      ...
      planejamento_avaliacao_postural = await database
        .select(
          "id",
          "id_planejamento as idPlanning",
          "pe as pe",
      ...
      planejamento_avaliacao_postural = await Promise.all(
        planejamento_avaliacao_postural,
      );
      ...
      planejamento_historico_familiar = await database
        .select(
          "id",
          "id_planejamento as idPlanning",
      ...
      planejamento_historico_familiar = await Promise.all(
        planejamento_historico_familiar,
      );
      ...
      planejamento_fracionamento_corporal = await database
        .select(
          "id",
          "id_planejamento as idPlanning",
      ...
      planejamento_fracionamento_corporal = await Promise.all(
        planejamento_fracionamento_corporal,
      );
      ...
      planejamento_cronograma = await database
        .select(
          "id",
          "id_planejamento as idPlanning",
          "horario as hour",
      ...
      planejamento_controle_diario = await database
        .select(
          "id",
          "id_planejamento as idPlanning",
      ...
      planejamento_controle_diario = await Promise.all(
        planejamento_controle_diario,
      );
      ...
      planejamento_macrociclo = await database
        .select(
          "id",
      ...
      var results2Promise = planejamento_macrociclo.map(async (obj2) => {
        planejamento_macrociclo_mes = await database
          .select(
            "id",
        ...
        planejamento_macrociclo_mes = planejamento_macrociclo_mes.map(
          async (obj3) => {
            planejamento_macrociclo_mes_dados = await database
              .select(
                "id",
            ...
            planejamento_macrociclo_mes_dados =
              planejamento_macrociclo_mes_dados.map(async (obj4) => {
                planejamento_macrociclo_mes_dados_caracateristicas =
                  await database
                    .select(
                      "id",
                ...
                obj4.planningMacrocycleMonthDatasCharacteristics =
                  await Promise.all(
                    planejamento_macrociclo_mes_dados_caracateristicas,
                  );
            ...
            obj3.planningMacrocycleMonthDatas = await Promise.all(
              planejamento_macrociclo_mes_dados,
            );
        ...
        obj2.planningMacrocycleMonths = await Promise.all(
          planejamento_macrociclo_mes,
        );
  ...
  async New(request, response) {
    const { studentId, month1, month2, month3, profile } = request.body;
    console.log(studentId, month1, month2, month3, profile);
    //await database.raw('CALL stp_cdpd_planejamento_criacao (?,?,?,?,?)',[Number(studentId),month1,month2,month3,profile])
    await database
      .raw("exec stp_cdpd_planejamento_criacao ?,?,?,?,?", [
        studentId,
        month1,
        month2,
        month3,
      ...
      .then(async (data) => {
        var planejamento,
          ...
          planejamento_macrociclo_mes_dados_caracateristicas;
        planejamento = await database
          .where({ id: data[1].id_planejamento })
          .select(
            "id",
            "descricao as description",
            "ano as year",
        ...
        var resultsPromise = planejamento.map(async (obj) => {
          ...
          obj.planningPathologicalHistories =
            planejamento_historico_patologico[0];
          ...
          obj.planningFuncionalEvaluations =
            planejamento_avaliacao_funcional[0];
          ...
          obj.planningBodyFractionations =
            planejamento_fracionamento_corporal[0];
                  ...
                  planejamento_macrociclo_mes_dados.map(async (obj4) => {
                    planejamento_macrociclo_mes_dados_caracateristicas =
                      await database
                        ...
                        .where({
                          id_planejamento_macrociclo_mes_dados: obj4.id,
                        });
    ...
    if (Object.keys(planningPathologicalHistories).length !== 0)
      await database
        ...
        .update({
          ...
          submetido_cirurgia_qual:
            planningPathologicalHistories.submetidoCirurgiaQual,
          uso_medicamentos: planningPathologicalHistories.usoMedicamentos,
          uso_medicamentos_qual:
            planningPathologicalHistories.usoMedicamentosQual,
          apresenta_lesao: planningPathologicalHistories.apresentaLesao,
          apresenta_lesao_qual:
            planningPathologicalHistories.apresentaLesaoQual,
          apresenta_alergia: planningPathologicalHistories.apresentaAlergia,
          apresenta_alergia_qual:
            planningPathologicalHistories.apresentaAlergiaQual,
          apresenta_doenca_ou_sintoma:
            planningPathologicalHistories.apresentaDoencaOuSintoma,
          apresenta_doenca_ou_sintoma_qual:
            planningPathologicalHistories.apresentaDoencaOuSintomaQual,
          tpm: planningPathologicalHistories.tpm,
          tpm_outras_manifestacoes:
            planningPathologicalHistories.tpmOutrasManifestacoes,
          ...
          fumante_a_quanto_tempo:
            planningPathologicalHistories.fumanteAQuantoTempo,
          usa_bebida_alcolicas:
            planningPathologicalHistories.usaBebidaAlcolicas,
          usa_bebida_alcolicas_com_que_frequencia:
            planningPathologicalHistories.usaBebidaAlcolicasComQueFrequencia,
          faz_dieta_para_que: planningPathologicalHistories.fazDietaParaQue,
          faz_dieta_quanto_tempo:
            planningPathologicalHistories.fazDietaQuantoTempo,
          dieta_consumo_diario_calorias:
            planningPathologicalHistories.dietaConsumoDiarioCalorias,
        })
        .table("planejamento_historico_patologico");
    ...
    if (Object.keys(planningFuncionalEvaluations).length !== 0)
      await database
        ...
        .update({
          ...
          agachamento_toracica:
            planningFuncionalEvaluations.agachamentoToracica,
          ...
          agachamento_tornozelo:
            planningFuncionalEvaluations.agachamentoTornozelo,
          ombro_esquerdo: planningFuncionalEvaluations.ombroEsquerdo,
          ombro_direito: planningFuncionalEvaluations.ombroDireito,
          rotacao_tronco_esquerdo:
            planningFuncionalEvaluations.rotacaoTroncoEsquerdo,
          rotacao_tronco_direto:
            planningFuncionalEvaluations.rotacaoTroncoDireito,
        })
        .table("planejamento_avaliacao_funcional");
      ...
      await database
        ...
        .update({
          ...
          circunferencia_abdominal:
            planningBodyFractionations.circunferenciaAbdominal,
          biceps_relaxado_direito:
            planningBodyFractionations.bicepsRelaxadoDireito,
          biceps_relaxado_esquerdo:
            planningBodyFractionations.bicepsRelaxadoEsquerdo,
          biceps_contraido_direito:
            planningBodyFractionations.bicepsContraidoDireito,
          biceps_contraido_esquerdo:
            planningBodyFractionations.bicepsContraidoEsquerdo,
    ...
    if (Object.keys(planningMacrocycles.planningMacrocycleMonths).length !== 0)
      planningMacrocycles.planningMacrocycleMonths.map(async (pmm) => {
        ...
        pmm.planningMacrocycleMonthDatas.map(async (pmmd) => {
          ...
          pmmd.planningMacrocycleMonthDatasCharacteristics.map(
            async (pmmdc) => {
              await database
                .where({ id: pmmdc.id })
                .update({
                  id_planejamento_macrociclo_mes_dados:
                    pmmdc.idPlanningMacrocycleMonthDatas,
